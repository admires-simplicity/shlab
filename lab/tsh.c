...
volatile sig_atomic_t fg_terminated_pid = 0;
...
void eval(char *cmdline) 
{
  ...
  if (pid == 0) { // child
    Setpgid(0, 0); /* puts child in new process group whose ID is
		      identical to child's PID, ensuring only shell
		      is in foreground group. */
    Sigprocmask(SIG_SETMASK, &prev_mask, NULL); //unblock SIGCHLD
    Execve(argv[0], argv, environ);
    //Execve doesn't return
  }
  else { // parent
    //add child job to jobs list
    if (!addjob(jobs, pid, bg ? BG : FG, cmdline)) {
      app_error("addjob error"); // It seems to me like it would be better to 
...
void do_bgfg(char **argv) 
{
  ...
  // get job and handle invalid jid/pid
  struct job_t *job;
  if (has_jid) {
  ...
  if (strcmp(argv[0], "fg") == 0) { // handle fg command
    job->state = FG;
    kill(-job->pid, SIGCONT);
    waitfg(job->pid);
  } else if (strcmp(argv[0], "bg") == 0) { // handle bg command
    job->state = BG;
    printbg(job);
    kill(-job->pid, SIGCONT);
  } else {
...
void waitfg(pid_t pid)
{
  ...
  //block SIGCHLD for good measure
  Sigprocmask(SIG_BLOCK, &mask, &prev_mask);

  //unblock SIGCHLD in mask for Sigsuspend
...
void sigchld_handler(int sig) 
{
  for (int i = 0; i < MAXJOBS; ++i) { // reaps all zombies
    if (jobs[i].pid != 0) { // job exists
      int status;
      pid_t pid = Waitpid(jobs[i].pid, &status, WNOHANG | WUNTRACED);
      // WNOHANG: do not block if child has not changed state, just return 0.
      // WUNTRACED: return pid if child stopped.
      if (pid && (WIFEXITED(status) || WIFSIGNALED(status))) {
        // child terminated normally or terminated in response to signal
        if (jobs[i].state == FG) {
          fg_terminated_pid = pid; // so waitfg can terminate
        }
...
void sigint_handler(int sig) 
{
  ...
  if (fg_pid) {
    kill(-fg_pid, SIGINT); // send SIGINT to foreground process group
  }
